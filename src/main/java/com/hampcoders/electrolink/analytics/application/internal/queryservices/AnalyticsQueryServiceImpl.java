    ...
    public List<TechnicianRevenueResource> handle(GetTechnicianRevenueQuery query) {
        ...
        return operations.stream()
            .collect(Collectors.groupingBy(op -> {
                ...
                return new AbstractMap.SimpleEntry<>(completedDate.getYear(), completedDate.getMonthValue());
            }))
            .entrySet().stream()
            .map(entry -> {
                var year = entry.getKey().getKey();
                ...
                if (periodStart.isBefore(cutoff)) return null;

                var opsInPeriod = entry.getValue();
                var totalRevenue = opsInPeriod.stream()
                    .mapToDouble(op -> {
                        var request = requestRepository.findById(op.getRequestId().requestId()).orElse(null);
