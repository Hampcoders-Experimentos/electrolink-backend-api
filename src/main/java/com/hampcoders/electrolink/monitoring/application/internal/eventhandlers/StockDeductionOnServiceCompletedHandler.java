...
public class StockDeductionOnServiceCompletedHandler {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(StockDeductionOnServiceCompletedHandler.class);
    ...
    public void onServiceCompleted(ServiceCompletedEvent event) {
        ...
        if (serviceIdOpt.isEmpty()) {
            LOGGER.warn("Request not found for ID: {}. Cannot perform stock deduction.", event.requestId());
            return;
        }

        String serviceId = serviceIdOpt.get();
        if (serviceId == null || serviceId.isBlank()) {
            LOGGER.warn("Request {} has no serviceId associated. Skipping stock deduction.", event.requestId());
            return;
        }

        List<ServiceComponentRequirement> components = sdpContextFacade.fetchServiceComponentRequirements(
        ...
        for (ServiceComponentRequirement componentQty : components) {
            try {
                ...
                LOGGER.info("Deducted {} units of component {} from technician {} inventory. New quantity: {}",
                        quantity, componentId, technicianId, newQuantity);

            } catch (NumberFormatException e) {
                LOGGER.warn("Invalid componentId format: {}. Skipping deduction.", componentQty.componentId());
            } catch (Exception e) {
